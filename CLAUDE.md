...
- Python 3.11+
- GitHub Actions cron, two workflows. Build: every day at 02:15 UTC (after the US close and after the SEC stops accepting filings for the day). It builds the edition for that UTC date, covering the last finished US trading day, and picks the edition type (weekday, Saturday, Sunday) from the date. Send: runs every hour, and sends the latest edition to each user whose local time has reached their send hour and who has not received it yet (tracked in state.json). Public repos get free Actions minutes, so hourly runs cost nothing. GitHub cron can run late and that is fine.
